...
$exitCode = 0
$SQLwLogin = "$SQLcmd -U $DBUser -P $DBPassword"
Get-ChildItem $path | Foreach-Object {
  $hashObj = Get-FileHash $_.FullName -Algorithm MD5
  $hash = $hashObj.hash
  #Store new scripts in the DB
  $output = & $SQLwLogin -S $DBServer -b -Q "IF NOT EXISTS(SELECT 1 FROM [MinistryPlatform].[dbo].[cr_Scripts] WHERE [MD5] = '$hash') INSERT INTO [MinistryPlatform].[dbo].[cr_Scripts] ([Name] ,[MD5]) VALUES ('$_','$hash')"
  if($LASTEXITCODE -ne 0){
		echo "File: $_"
		echo "Error: $output"
		$exitCode = $LASTEXITCODE
	} elseif($output -eq "(1 rows affected)"){#If a new script was stored then run it
		$output = & $SQLwLogin -I -S $DBServer -b -i $path\$_
		if($LASTEXITCODE -ne 0){
			echo "File: $_"
			echo "Error: $output"
			$exitCode = $LASTEXITCODE
		} else {#If the new script executed well mark it as executed
			$output = & $SQLwLogin -S $DBServer -b -Q "Update [MinistryPlatform].[dbo].[cr_Scripts] set executed=1 where [MD5] = '$hash'"
			if($LASTEXITCODE -ne 0){
				echo "File: $_"
				echo "Error: $output"
				$exitCode = $LASTEXITCODE
