...
Get-ChildItem $args | Foreach-Object {
  ...
  $hash = $hashObj.hash
  $output = & sqlcmd -S mp-int-db.cloudapp.net -b -Q "IF NOT EXISTS(SELECT 1 FROM [MinistryPlatform].[dbo].[cr_Scripts] WHERE [MD5] = '$hash') INSERT INTO [MinistryPlatform].[dbo].[cr_Scripts] ([Name] ,[MD5]) VALUES ('$_','$hash')"
  if($LASTEXITCODE -ne 0){
		echo "Error: $output"
		$exitCode = $LASTEXITCODE
	} else {
  	$output = & sqlcmd -S mp-int-db.cloudapp.net -b -i $_
	if($LASTEXITCODE -ne 0){
		echo "Error: $output"
		$exitCode = $LASTEXITCODE
	}
  	$output = & sqlcmd -S mp-int-db.cloudapp.net -b -Q "Update [MinistryPlatform].[dbo].[cr_Scripts] set executed=1 where [MD5] = '$hash'"
	if($LASTEXITCODE -ne 0){
		echo "Error: $output"
		$exitCode = $LASTEXITCODE
	}
