...
BEGIN
  ...
  DECLARE @DonorIdTbl TABLE (RowID int identity(1,1), ID INT, New_Stmt_Type INT, Prev_Stmt_Type INT)
  ...
  DECLARE @AuditLogTbl TABLE (ARowID int identity(1,1),AID INT)
  ...
  FROM @AuditLogTbl A 
    INNER JOIN @DonorIdTbl T on T.RowID = A.ARowID    
