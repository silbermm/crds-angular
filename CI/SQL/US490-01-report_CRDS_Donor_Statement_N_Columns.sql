...
CREATE TABLE #D (Donor_ID INT, Pledge Money, Given_to_Pledge Money, Statement_ID Varchar(11))
CREATE TABLE #PLAmt (Pledge_ID INT, Amount Money)
INSERT INTO #PLAmt (Pledge_ID, Amount)
SELECT PL.Pledge_ID, SUM(DD.Amount) AS Amount
...
WHERE PL.Pledge_Campaign_ID = @CampaignID
GROUP BY PL.Pledge_ID

CREATE INDEX IX_Temp_PlAmt_PledgeID ON #PLAmt(Pledge_ID)

CREATE TABLE #PL(Pledge_ID INT, Donor_ID INT, Total_Pledge MONEY, Statement_Type_ID INT, Household_ID INT, Contact_ID INT, Given Money)
INSERT INTO #PL (Pledge_ID, Donor_ID, Total_Pledge, Statement_Type_ID, Household_ID, Contact_ID, Given)
SELECT PL.Pledge_ID
, Do.Donor_ID
, PL.Total_Pledge
...
WHERE Pledge_Campaign_ID = @CampaignID
GROUP BY PL.Pledge_ID
, Do.Donor_ID
, PL.Total_Pledge
, Do.Statement_Type_ID
...
CREATE INDEX IX_TempPL_HouseholdID ON #PL(Household_ID)
	...
	BEGIN
	INSERT INTO #D (Donor_ID, Pledge, Given_to_Pledge, Statement_ID)
	SELECT Do.Donor_ID
...
SELECT Donation_ID
	...
	, Payment_Type
	, Payment_Type_ID
	, CASE WHEN Payment_Type_ID = 6 THEN 'Non-Cash/Asset' ELSE ISNULL(Item_Number,Payment_Type) END AS Item_Number
	--, Item_Number
	, Fund_Name
	, Donation_Detail
	, Donor_Name
...
FROM (SELECT Top 100 PERCENT D.Donation_ID
	...
	, On_Pledge = CASE WHEN ISNULL(DD.Pledge_ID,0)=0 THEN '' ELSE 'Yes' END
	...
	, #D.Statement_ID-- = CASE WHEN @CombineFamily = 0 OR C.Household_ID IS NULL OR Do.Statement_Type_ID = 1  THEN 'C' + CONVERT(Varchar(10),C.Contact_ID) ELSE 'F' + CONVERT(Varchar(10),H.Household_ID) END
	, Do.Envelope_No
