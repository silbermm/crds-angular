...
​
 ...
 update dp_Contact_Publications set unsubscribed = 1 where Third_Party_Contact_ID in(select email from #MPEMAILREJECTS where reason = 'spam' )
 -- constant contact unsubscribe
 update dp_Contact_Publications set unsubscribed = 1 
	where Third_Party_Contact_ID in(select emailaddr_Home from #CCEMAILUNSUBSCRIBE where emailstatus_Home = 'Unsubscribed'
...
​
 ...
 insert into cr_tmp_hardbounce
	select Third_Party_Contact_ID,getdate(),'mandrill' from dp_Contact_Publications cp  where Third_Party_Contact_ID in(select email from #MPEMAILREJECTS where reason = 'hard-bounce' )
 
 --bounced constant contact addresses
 insert into cr_tmp_hardbounce
	select Third_Party_Contact_ID,getdate(),'constant contact' from dp_Contact_Publications cp  
					where Third_Party_Contact_ID in(select emailaddr_Home from #CCEMAILBOUNCE where emailaddr_Home is not null 
