...
Set @contactID = (select top 1 contact_id from contacts where email_address = 'rtheil+wife@crossroads.net' and last_name = 'Theil-Test');
...
WHERE email_address = 'rtheil+wife@crossroads.net' and last_name = 'Theil-Test';
...
--Just get rid of this so we can delete Wife's old contact record
...
--Delete all donations for Wife's trip pledge program.
...
--Delete all of Wife's donations.
...
--Delete userAccount
delete from dp_user_roles where user_id = (select user_id from dp_users where user_email = 'rtheil+wife@crossroads.net');

delete from dp_users where user_email = 'rtheil+wife@crossroads.net';

--Delete Wife's old contact record
DELETE FROM [dbo].Contacts where contact_id = (select top 1 contact_id from contacts where email_address = 'rtheil+wife@crossroads.net' and last_name = 'Theil-Test');
GO