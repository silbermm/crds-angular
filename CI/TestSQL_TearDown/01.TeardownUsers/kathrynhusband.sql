...
Set @contactID = (select top 1 contact_id from contacts where email_address = 'kcradduck+husband@crossroads.net' and last_name = 'Cradduck-Test');
...
WHERE email_address = 'kcradduck+husband@crossroads.net' and last_name = 'Cradduck-Test';
...
--Just get rid of this so we can delete Husband's old contact record
...
--Delete all donations for Husband's trip pledge program.
...
--Delete userAccount
delete from dp_user_roles where user_id = (select user_id from dp_users where user_email = 'kcradduck+husband@crossroads.net');

delete from dp_users where user_email = 'kcradduck+husband@crossroads.net';

--Delete Husband's old contact record
DELETE FROM [dbo].Contacts where contact_id = (select top 1 contact_id from contacts where email_address = 'kcradduck+husband@crossroads.net' and last_name = 'Cradduck-Test');
GO